...
The functions $A(\lambda)$ and $k(\lambda)$ are modeled with B-splines, with the number of knots adjustable by the user. `raccoon` provides a functionality for the user to determine the appropriate number of knots using model selection criteria such as the Bayesian information criterion (BIC) or the minimum *a posteriori* chi-squared metric ($\chi^2_{\rm MAP}$). The best-fit values for the linear coefficients (i.e, $c_1, c_2, c_3, p_0, \dots, p_N$) and non-linear parameters (i.e., $a_1$, $a_2$, $b$ and the coefficients of the B-spline basis functions) are determined by minimizing the chi-squared quantity
$$
...
`raccoon` additionally allows the user to easily loop through multiple spaxels within the datacube, optionally within a user-specified spatial region. The user can also set a detection threshold for the wiggle signal before `raccoon` makes a correction on the given spectra.


`raccoon` offers several advantages over previously available scripts and routines. Earlier tools [@Perna23; @Dumont25] extract the wiggle signal by subtracting the single-spaxel spectrum from a template spectrum (similar to or same as the one in \autoref{template}) that is fitted to the data without accounting for wiggles. This approach can potentially result in  loss of a fraction of the wiggle signal during numerical fitting. In contrast, `raccoon` fits the template spectrum and the parametric wiggle model $W(\lambda)$ simultaneously to the data, eliminating the need for an intermediate extraction step and preserving the full wiggle signal. While previous scripts perform fitting separately in multiple wavelength slices, `raccoon` fits across the entire wavelength range at once. This makes `raccoon` more robust to gaps in the fitted spectrum, whether excluded by manual masking or outlier rejection. Another important distinction is that earlier tools apply an additive correction, whereas `raccoon` uses a multiplicative correction. Since both approaches derive the correction empirically from the data, this difference should not affect practical performance. However, as the wiggles manifest as a multiplicative effect on the spectra [@Law23], `raccoon`'s implementation is physically motivated. Additionally, `raccoon` is uniquely installable via the `pip` command to allow a more flexible importability into a Python script or notebook, making it more user-friendly and portable compared to its peers.
